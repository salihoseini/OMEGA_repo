...
function show_order_in_details(order_info, order_id, order_time, order_price, j_day, day, month, year, time, table_number, user_id, order_note) {
    ...
    if(order_exist) {
        var table_header = "<div id=\'preparing-order" + order_id + "\' class=\'order\'>\n" +
            "<span style=\'display: none\' id=\'user-id" + order_id + "\'>" + user_id + "</span> " +
            "<table class=\"invoice-table\" style=\"background-color: #0D47A1; color: white\">\n" +
            "                                <tr style=\"position: relative\">\n" +
            "                                    <th><span>زمان باقیمانده تا آماده شدن سفارش : </span>\n" +
            "                                        <span>" + order_time + "</span>\n" +
            "                                        <span>دقیقه</span>\n" +
            "                                        <img src=\"../static/img/add.png\" class=\"fit-icons add-margin-right\">\n" +
            "                                        <img id=\'complete-order" + order_id + "\' src=\"../static/img/unblock.png\" class=\"fit-icons confirm-margin-right\">\n" +
            "                                    </th>\n" +
            "                                </tr>\n" +
            "                            </table>";
        var table_content =
...
function get_orders() {
        ...
        socket.onmessage = function (e) {
            ...
            if (json_data.type !== 'websocket.disconnect') {
                ...
                else if (src === "payment" && dest === "kitchen") {
                    //hide_order(order_id);
